...
task = Task.init(
    project_name=os.environ["CLEARML_PROJECT_NAME"],
...
def save_image_grid(
    images, filename, drange, grid_size, iteration=None, t1=None, t2=None
):
    lo, hi = drange
    gw, gh = grid_size
        ...
        if done or cur_tick < 0 or cur_nimg >= tick_start_nimg + kimg_per_tick * 1000:
            ...
            if network_snapshot_ticks is not None and (
                ...
                task.upload_artifact(
                    f"network-snapshot-{cur_nimg // 1000:06d}.pkl", (G, D, Gs)
                )
