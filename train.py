...
def setup_training_options(
    ...
    if cfg == "auto":
        ...
        cfg == "auto_tuned"
    ):  # custom config, modification of the standard one but partially based on paper256
        desc += f"{gpus:d}"
        spec.ref_gpus = gpus
        spec.mb = max(
            min(gpus * min(4096 // res, 32), 64), gpus
    ...
    args.G_args.num_fp16_res = (
        args.D_args.num_fp16_res
    ) = 4  # enable mixed-precision training
    args.G_args.conv_clamp = (
        args.D_args.conv_clamp
    ...
    if p is not None:
        ...
        if resume is None and aug != "fixed":
            raise UserError("--p can only be specified with --aug=fixed")
        if resume is None and not 0 <= p <= 1:
